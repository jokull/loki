...
// 1x1 transparent PNG
const PNG = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+M8AAAMCAoD3vAAAAABJRU5ErkJggg==";
async function main(){
  ...
  KEY=su.apiKey;ORIGIN=su.siteUrl;mcpUrl=su.mcpUrl;console.log("Site:",ORIGIN);
  ...
  await tool("site_write",{path:"functions/up.ts",source:`import { serverFn } from "loki/runtime";
export const upload = serverFn({ method: "POST" }).validator((i)=>({ b64:String(i.b64||"") })).handler(async ({ data, env }) => {
  ...
  const pub=await tool("publish_site",{message:"uploads"});const v=(/Published v(\d+)/.exec(pub)||[])[1];
  ok("publish",!!v,"v"+v);await new Promise(r=>setTimeout(r,1000));
  const res=await(await fetch(`${ORIGIN}/__fn/v${v}/functions%2Fup.ts%23upload`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({data:{b64:PNG}})})).json();
  ok("env.UPLOADS.put returns url",res&&res.ok&&res.url==="/__uploads/pics/test.png",JSON.stringify(res));
  await new Promise(r=>setTimeout(r,500));
  const img=await fetch(`${ORIGIN}/__uploads/pics/test.png`);
  const ctype=img.headers.get("content-type");const buf=new Uint8Array(await img.arrayBuffer());
  ok("uploaded file serves at /__uploads/ with content-type",img.status===200&&ctype==="image/png"&&buf.length>0,`status=${img.status} ct=${ctype} bytes=${buf.length}`);
  const passed=results.filter(Boolean).length;console.log(`\n${passed}/${results.length} — ${ORIGIN}`);process.exit(passed===results.length?0:1);
}
