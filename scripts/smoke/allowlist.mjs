const APEX="https://loftur.app";const SUB="allowlab"+Math.floor(Date.now()/1000)%100000;let mcpUrl,KEY,ORIGIN,rid=1;
...
async function main(){
  ...
  await tool("site_write",{path:"loki.config.json",source:`{ "writableModels": [], "allowedHosts": ["example.com"] }`});
  await tool("site_write",{path:"routes/index.tsx",source:`export default function H(){return <main>ok</main>;}`});
  await tool("site_write",{path:"functions/net.ts",source:`import { serverFn } from "loki/runtime";
...
`});
  const pub=await tool("publish_site",{message:"allowlist"});const v=(/Published v(\d+)/.exec(pub)||[])[1];
  ok("publish",!!v,"v"+v);await new Promise(r=>setTimeout(r,900));
