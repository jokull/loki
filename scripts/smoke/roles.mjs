...
async function signIn(vnum) {
  const mm = await (
    await fetch(APEX + "/__authmagic", {
      method: "POST",
      headers: { "content-type": "application/json", authorization: "Bearer " + WRITE_KEY },
      body: JSON.stringify({ sub: SUB, email: USER, redirectTo: "/" }),
    })
  ).json();
  const verify = await fetch(mm.link, { redirect: "manual" });
  const cookie = cookieOf(verify.headers.get("set-cookie"));
  const who = await (
    await fetch(`${ORIGIN}/__fn/v${vnum}/functions%2Ftest.ts%23whoami`, {
      headers: { cookie: "loki_session=" + cookie },
    })
  ).json();
  return who.user;
}
...
async function main() {
  ...
  ok(
    "list_users shows the user as admin",
    new RegExp(USER + "\\s+admin").test(list),
    list.split("\n")[0],
  );
