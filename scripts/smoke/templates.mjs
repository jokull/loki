...
async function main() {
  ...
  ok(
    "scaffold members writes files",
    /Scaffolded "members"/.test(scaff) && /routes\/members.tsx/.test(scaff),
  );
  const pub = await tool("publish_site", { message: "members tpl" });
  ...
  const pub2 = await tool("publish_site", { message: "bio" });
  const v2 = (/Published v(\d+)/.exec(pub2) || [])[1];
