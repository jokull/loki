    ...
    <Shell width="prose" className="flex flex-col gap-10">
      ...
      <Section title="3. What the agent works with">
        <ul className="flex list-disc flex-col gap-2 pl-5 text-sm text-muted-foreground marker:text-link">
          <li>
            <b>Content</b> — models, fields, records, publishing, assets over a typed GraphQL +
            schema API (DatoCMS-style). Localized content via built-in locales.
          </li>
          <li>
            <b>Feature database</b> — design relational tables at runtime with{" "}
            <code>feature_migrate</code>; query with Drizzle over <code>env.FEATURES_SQL</code>.
          </li>
          <li>
            <b>Routes, islands & serverFns</b> — file-based routing, SSR Preact, hydrated islands,
            typed server functions. No bundler, no deploy step.
          </li>
          <li>
            <b>Auth</b> — passwordless magic-link login for your visitors; <code>user</code> is
            injected into every loader and serverFn.
          </li>
          <li>
            <b>Secrets & outbound</b> — <code>env.SECRETS.get()</code> for API keys, mediated{" "}
            <code>fetch()</code> to any host — call Stripe, Resend, anything.
          </li>
          <li>
            <b>npm imports</b> — import any workerd-compatible package; resolved and snapshotted at
            write time.
          </li>
        </ul>
      </Section>
      ...
      <Section title="Owner vs editor">
        <p className="text-sm text-muted-foreground">
          ...
          <b>editor tokens</b> from your dashboard so a content editor can maintain content and
          upload images over their own MCP client — no schema or code access.
        </p>
      </Section>
